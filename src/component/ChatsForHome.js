...
function ChatsForHome({set_dark_mode,user_data_email,user_name}) {
    ...
    const fetch_users_data = async () => {
        try {
            const response = await fetch('https://node-test-backend-ten.vercel.app/get_users');
            if (!response.ok) {
    ...
    const fetch_chat_data_by_room_id = async () => {
        try {
          const response = await fetch('https://node-test-backend-ten.vercel.app/api/get_chat', {
            method: 'POST',
            headers: {
      ...
      const sendNewMessage = async (newMessage) => {
        try {
            const response = await fetch('https://node-test-backend-ten.vercel.app/api/send_message', { // Update the endpoint as needed
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
